...
func (inst *instance) boot() error {
	...
	inst.runFfx(5*time.Minute, ffxCommandOpts{isolated: true, startDaemon: false}, "emu", "stop", inst.name)
...
func (inst *instance) startFuchsiaVM() error {
	if _, err := inst.runFfx(
		5*time.Minute,
		ffxCommandOpts{isolated: true, startDaemon: false},
		"emu", "start", "--headless",
...
func (inst *instance) startSshdAndConnect() error {
	// Start the ffx daemon as required for `ffx component run`.
	opts := ffxCommandOpts{isolated: true, startDaemon: true}
	if _, err := inst.runFfx(
		5*time.Minute,
		opts,
		"--target",
...
func (inst *instance) runFfx(timeout time.Duration, opts ffxCommandOpts, args ...string) ([]byte, error) {
	if inst.debug {
		isolation := "without"
		if opts.isolated {
			isolation = "with"
	...
	cmd := inst.ffxCommand(opts, inst.ffxBinary, args...)
	cmd.Stderr = os.Stderr
...
func (inst *instance) getFfxConfigValue(key string) (string, error) {
	rawValue, err := inst.runFfx(
		30*time.Second,
		ffxCommandOpts{isolated: false, startDaemon: false},
		"config", "get", key)
	if err != nil {
		return "", err
	}
